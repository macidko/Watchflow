      ...
      {showImage ? (
        <>
          ...
          <div className="absolute bottom-0 left-0 w-full p-3 flex flex-col">
            <h3 className="text-white font-bold text-lg mb-1 line-clamp-1 drop-shadow-lg">{item.title}</h3>
            <p className="text-zinc-200 text-xs line-clamp-2 mb-1 drop-shadow">{item.description}</p>
            ...
            <div className="flex items-center justify-between w-full">
              {item.year && (
                <span className="text-zinc-300 text-xs drop-shadow">{item.year}</span>
              )}
              {item.rating && (
                <div className="flex items-center">
                  ...
                  <span className="text-yellow-400 text-xs font-medium drop-shadow">{item.rating}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="w-full h-full flex flex-col items-center justify-center text-center p-4">
          ...
          <p className="text-zinc-400 text-xs line-clamp-2 mb-2">{item.description}</p>
          {/* Genre etiketleri */}
          {item.genre && item.genre.length > 0 && (
          ...
          <div className="flex items-center justify-between w-full mt-auto">
            ...
            {item.rating && (
              <div className="flex items-center">
                ...
                <span className="text-yellow-500 text-xs font-medium">{item.rating}</span>
              </div>
            )}
          </div>
        </div>
