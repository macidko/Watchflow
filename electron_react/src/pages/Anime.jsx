...
const Anime = () => {
  ...
  const { viewMode, toggleViewMode } = useViewMode('anime');
  ...
  const handleCardMove = (cardItem, fromSliderId, toSliderId) => {
    ...
    const success = moveContentBetweenStatuses(cardItem, fromStatusId, toStatusId, 'anime');
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <h1 style={{ fontSize: 30, fontWeight: 600, color: 'var(--primary-text)' }}>Anime</h1>
              <p style={{ fontSize: 18, color: 'var(--secondary-text)' }}>İzlediğin ve izleyeceğin animeleri organize et</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12 }}>
                ...
                <span style={{ fontSize: 14, color: 'var(--text-muted)', fontWeight: 400 }}>
                  {sliders.reduce((total, slider) => total + slider.items.length, 0)} Anime
                </span>
              </div>
            </div>
          ...
          {sliders.length === 0 ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 384 }}>
              <div style={{ textAlign: 'center', maxWidth: 400 }}>
                ...
                <h2 style={{ fontSize: 20, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 16 }}>Henüz Anime Yok</h2>
                <p style={{ color: 'var(--secondary-text)', marginBottom: 24 }}>
                  Anime koleksiyonunu oluşturmaya başla! Arama yaparak animeler ekleyebilir ve listeler oluşturabilirsin.
                </p>
                <SearchButton 
                  category="anime" 
